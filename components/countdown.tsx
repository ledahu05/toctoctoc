...
function calculateTimeFraction(time: number, timeLeft: number): number {
  const rawTimeFraction = timeLeft / time;
  return rawTimeFraction - (1 / time) * (1 - rawTimeFraction);
}

const Countdown: React.FC<{ time: number; timeLeft: number }> = ({ time, timeLeft }) => {
  ...
  const formatTime = useCallback((timeLeft) => {
  ...
  return (
    <CountdownContainer className="base-timer mt-4" remainingPathColor={remainingPathColor}>
      <svg className="base-timer__svg" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
        <g className="base-timer__circle">
          <circle className="base-timer__path-elapsed" cx="50" cy="50" r="45" />
          <path
            id="base-timer-path-remaining"
            strokeDasharray={`${timeFraction.toFixed(0)} ${FULL_DASH_ARRAY}`}
